...
import { assets, ketchupDipsSpreadsProducts } from "../assets/assets";
import { useAppContext } from "../context/AppContext";
import { Link } from "react-router-dom";

const KetchupDipsSpreads = () => {
    const { addToCart, removeFromCart, cartItems, navigate, wishlist, toggleWishlist } = useAppContext();
    const [filter, setFilter] = useState("All");
    const [filteredProducts, setFilteredProducts] = useState(ketchupDipsSpreadsProducts);

    const categories = ["All", "Ketchup & Sauces", "Dips", "Sweet Spreads"];

    useEffect(() => {
        if (filter === "All") {
            setFilteredProducts(ketchupDipsSpreadsProducts);
        } else {
            setFilteredProducts(
                ketchupDipsSpreadsProducts.filter((product) => product.subCategory === filter)
            );
        }
    }, [filter]);
    ...
    return (
        <div className="mt-20 pb-20">
            ...
            <div className="flex items-center gap-2 text-gray-500 text-sm mb-6">
                ...
                <span>&gt;</span>
                <span className="text-indigo-600 font-medium">Ketchup, Dips & Spreads</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-800">
                        Ketchup, Dips & Spreads
                    </h1>
                    <p className="text-gray-500 mt-2">
                        Enhance your meals with our selection of ketchups, dips, and delicious spreads.
                    </p>
                </div>
                <button
...
export default KetchupDipsSpreads;
