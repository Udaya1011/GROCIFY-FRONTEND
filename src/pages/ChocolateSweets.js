...
import { assets, chocolateSweetsProducts } from "../assets/assets";
import { useAppContext } from "../context/AppContext";
import { Link } from "react-router-dom";

const ChocolateSweets = () => {
    const { addToCart, removeFromCart, cartItems, navigate, wishlist, toggleWishlist } = useAppContext();
    const [filter, setFilter] = useState("All");
    const [filteredProducts, setFilteredProducts] = useState(chocolateSweetsProducts);

    const categories = ["All", "Chocolates", "Candies", "Traditional Sweets", "Premium & Healthy"];

    useEffect(() => {
        if (filter === "All") {
            setFilteredProducts(chocolateSweetsProducts);
        } else {
            setFilteredProducts(
                chocolateSweetsProducts.filter((product) => product.subCategory === filter)
            );
        }
    }, [filter]);
    ...
    return (
        <div className="mt-20 pb-20">
            ...
            <div className="flex items-center gap-2 text-gray-500 text-sm mb-6">
                ...
                <span>&gt;</span>
                <span className="text-indigo-600 font-medium">Chocolate & Sweets</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-2">
                        Indulge in our collection of delicious chocolates, candies, and traditional sweets.
                    </p>
                </div>
                <button
...
export default ChocolateSweets;
